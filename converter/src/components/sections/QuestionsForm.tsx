...
import {QuestionsFormValues, formQuestionsSchema} from "../../schemas/zodSchema";
...
export function QuestionsForm({ formComponents, title }: QuestionsFormProps) {
    ...
    return (
        <form
            className="border-brand-bg flex flex-col h-full justify-center gap-2 border-2 px-8 py-5"
        ...
        >
            ...
            {formComponents.map((item) => {
                return (
                    <div className="flex items-center gap-3 w-full" key={item.name}>
                        <div className='grid grid-cols-[200px_1fr] w-full grid-rows-[55px]'>
                            ...
                            {item.type !== 'select' ? (
                                <FormInput
                                    ...
                                    rows={item.type === 'textarea' ? 2 : undefined}
                                />
                            ) : (
                                <FormSelect
                                    name={item.name}
            ...
            <FormControlLabel
                control={<Checkbox onChange={(event) => setChecked(event.target.checked)}/>}
